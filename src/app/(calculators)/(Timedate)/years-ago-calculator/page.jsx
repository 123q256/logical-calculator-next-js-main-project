...
const YearsAgoCalculator = () => {
  ...
  useEffect(() => {
    ...
    const htmlContent = Array.from({ length: 50 }, (_, i) => {
      ...
      const addedWeeksDate = subtractYears(today, index);
      ...
      return `
             <tr className="font-bold hover:bg-gray-600" style={{ fontWeight: 700 }}>
                 <td><time datetime="PT${index}W">${index} Years ago form today</time></td>
  ...
  function subtractYears(date, years) {
    ...
    result.setFullYear(result.getFullYear() - years); // --- Subtracting years here
    return result;
  }
                    ...
                    <div className="w-full bg-light-blue result  rounded-lg mt-4 overflow-auto">
                      <div className="flex flex-wrap">
                        ...
                        <p className="w-full text-center p-2">
                          What is <strong>{formData.tech_number}</strong> years
                          ago? The answer is{" "}
                          <strong>{result?.date_name}</strong>,{" "}
                          <strong>{result?.t_date}</strong>. It can be converted
                          into <strong>{result?.WeekOfYear}</strong> weeks. It
                          is also equivalent to{" "}
                          <strong>{result?.diffInMonths}</strong> months or
                          during these periods, there are{" "}
                          <strong>{result?.DayOfYear}</strong> days.
                        </p>
                      </div>
                    </div>
                  </div>
...
export default YearsAgoCalculator;
