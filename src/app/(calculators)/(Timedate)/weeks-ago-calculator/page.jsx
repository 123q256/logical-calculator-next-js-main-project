...
const WeeksAgoCalculator = () => {
  ...
  useEffect(() => {
    ...
    const htmlContent = Array.from({ length: 50 }, (_, i) => {
      ...
      const addedWeeksDate = substractWeeks(today, index);
  ...
  function substractWeeks(date, weeks) {
  ...
  const addFourWeeks = substractWeeks(today, 4);
  const addSixWeeks = substractWeeks(today, 6);
  const addEightWeeks = substractWeeks(today, 8);
  const addTenWeeks = substractWeeks(today, 10);
  ...
  const dateName12 = getDayName(addtwlWeeks);
  const formattedDate12 = formatDateString(addtwlWeeks);
  ...
  useEffect(() => {
    if (addFourWeeks) {
      ...
      const elementtwlWeeks = document.getElementById("addtwlWeeks");
      if (elementFour) {
        ...
        elementtwlWeeks.textContent = ` ${dateName12}, ${formattedDate12}`;
      }
    }
  }, [addFourWeeks]);
                  ...
                  <div className="rounded-lg  flex items-center justify-center">
                    <div className="w-full rounded-lg mt-4 overflow-auto">
                      ...
                      <p className="text-center p-2">
                        ...
                        total <strong>{result?.tech_weeksInYear}</strong> weeks
                        of the year. It also marks the day{" "}
...
export default WeeksAgoCalculator;
