...
import {
  useGetSingleCalculatorDetailsMutation,useTdeeCalculatorMutation
...
// country 
const metricCountries = ["United States", "Canada", "United Kingdom", "Pakistan"];
useEffect(() => {
  const countryName = formData.countryName || ""; // Set this from your user data
  const hightUnit =
    formData.tech_hightUnit || (metricCountries.includes(countryName) ? "ft/in" : "cm");
  setFormData((prev) => ({
    ...prev,
...
// country 
  ...
  const getBMICategoryStyle = (category) => {
    switch (category) {
      case 'Underweight': return 'text-teal-600';
      case 'Normal Weight': return 'text-green-700';
      case 'Overweight': return 'text-yellow-600';
      case 'Obesity': return 'text-red-600';
      case 'Severe Obesity': return 'text-red-700';
      default: return '';
    }
  };
...
const [showPercentError, setShowPercentError] = useState(false);
  ...
  const [weeklyCalories, setWeeklyCalories] = useState((result?.tech_tdee || 0) * 7);
  const [formulaLabel, setFormulaLabel] = useState(data?.payload?.tech_lang_keys?.['66'] || '');
  ...
  const activity = formData?.tech_activity ?? result?.tech_activity ?? 'Sedentary';
  const percent = Number(formData?.tech_percent ?? result?.tech_percent ?? 0);
  const height_cm = Number(formData?.tech_height_cm ?? result?.tech_height_cm ?? 0);
  ...
  useEffect(() => {
    if (bmr > 0 && tdee > 0) {
      ...
      setChartData({
        labels: [
          `BMR (${data?.payload?.tech_lang_keys?.["bmr"] || 'Basal Metabolic Rate'})`,
          `${data?.payload?.tech_lang_keys?.['81'] || 'Physical Activity'} (EAT + NEAT)`,
          `TEF (${data?.payload?.tech_lang_keys?.['82'] || 'Thermic Effect of Food'})`
        ],
        datasets: [
          {
            data: [
...
// Update DOM
  ...
  const getFormulaLabel = (key) => {
    if (key === 'katch') return data?.payload?.tech_lang_keys?.['68'] || 'Katch-McArdle';
    if (key === 'revised') return data?.payload?.tech_lang_keys?.['67'] || 'Revised Harris-Benedict';
  ...
  const chartData2 = {
    maintenance: {
      labels: [data?.payload?.tech_lang_keys['pro'], data?.payload?.tech_lang_keys['fat'], data?.payload?.tech_lang_keys['carb']],
      datasets: [
        {
          data: [
        ...
        <form className="row" onSubmit={handleSubmit}>
          <div className="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg space-y-6 mb-3">
               ...
               <div className="lg:w-[60%] md:w-[60%] w-full mx-auto ">
                <div className="grid grid-cols-12   gap-1 md:gap-4 lg:gap-4">
                   ...
                   <div className="col-span-12">
                    ...
                    <div className="col-span-6 md:col-span-6 lg:col-span-6  h_cm">
                      ...
                      <div className="relative w-full ">
                        ...
                        {dropdownVisible2 && (
                          <div className="absolute z-10 bg-white border border-gray-300 rounded-md w-auto mt-1 right-0">
                            {[
                              { label: data?.payload?.tech_lang_keys['60']+" (lbs)", value: "lbs" },
                              { label: data?.payload?.tech_lang_keys['61']+" (kg)", value: "cm" },
                            ].map((unit, index) => (
                              <p
                                key={index}
                  ...
                  {formData.tech_hightUnit == "ft/in"  &&(
                    <>
                    <div className="col-span-6 md:col-span-3 ft_in mr-2 ">
                    ...
                    <div className="col-span-6 md:col-span-3 ft_in mr-2 mt-1">
                    ...
                    <>
                    <div className="col-span-6 md:col-span-6 lg:col-span-6 w-full  h_cm">
                        ...
                        { data?.payload?.tech_lang_keys['height'] } (cm):
                          ...
                          <div className="relative w-full ">
                            ...
                            {dropdownVisible1 && (
                              <div className="absolute z-10 bg-white border border-gray-300 rounded-md w-auto mt-1 right-0">
                                {[
                                  { label: data?.payload?.tech_lang_keys['62']+" (ft/in)", value: "ft/in" },
                                  { label: data?.payload?.tech_lang_keys['63']+"(cm)", value: "cm" },
                                ].map((unit, index) => (
                                  <p
                                    key={index}
                                    className="p-2 hover:bg-gray-100 cursor-pointer"
                    ...
                    <div className="col-span-12 md:col-span-6 lg:col-span-6">
                        ...
                        <div className="mt-2">
                          <select
                            ...
                            onChange={handleChange}
                          >
                            <option value="Sedentary">{data?.payload?.tech_lang_keys["64"]} </option>
                            <option value="Lightly Active">{data?.payload?.tech_lang_keys["Lightly"]} </option>
                            <option value="Moderately Active">{data?.payload?.tech_lang_keys["Moderately"]} </option>
                            <option value="Very Active">{data?.payload?.tech_lang_keys["Very"]} </option>
                            <option value="Extremely Active">{data?.payload?.tech_lang_keys["Extremely"]} </option>
                          </select>
                        </div>
                    </div>
                      <div className="col-span-12 md:col-span-6 lg:col-span-6 percent ">
                            <label htmlFor="tech_percent" className="label">
                            {data?.payload?.tech_lang_keys["b_f"]}    <a title="Body Fat Percentage Calculator" href="/body-fat-percentage-calculator" className="text-blue font-s-12" target="_blank" rel="noopener"> { data?.payload?.tech_lang_keys['click'] }</a>:
                              </label>
                              <div className=" relative">
                              <input
                                type="number"
            ...
            <div className="col-span-12 px-2 mt-3 text-[14px]">
                <p><strong>Disclaimer:</strong> The TDEE calculator gives estimates using the Mifflin-St Jeor equation. These results are for informational purposes only. Consult a healthcare professional before making dietary or fitness changes.</p>
            </div>
            </div>
          </div>
                  ...
                  <div>
                    <ResultActions lang={data?.payload?.tech_lang_keys} />
                           ...
                           <div className="grid grid-cols-12 gap-2">
                            ...
                            <div className="col-span-8 md:col-span-7 lg:col-span-4 result_custom_inputs">
                              <select
                                name="formula_types"
                              ...
                              >
                                <option value="mifflin">{data?.payload?.tech_lang_keys['66']}</option>
                                <option value="revised">{data?.payload?.tech_lang_keys['67']}</option>
                                <option value="katch">{data?.payload?.tech_lang_keys['68']}</option>
                              </select>
                            </div>
                          </div>
                       ...
                       <div className="w-full bg-white col-span-12 rounded-lg ">
                            <div className="bg-white rounded-lg px-4 shadow-lg">
                              <p className="text-center text-xl font-bold mt-3 text-blue-600">
                                <strong>{data?.payload?.tech_lang_keys['70']} (TDEE)</strong>
                              </p>
                              ...
                              <div className="lg:flex md:flex justify-between">
                                <div className="lg:w-1/2 lg:pr-4 md:pr-4 mt-3 my-auto">
                                  <div className="bg-gray-100 rounded-lg text-center p-6 shadow-md">
                                    <p>
                                      <b className="text-green-600 text-5xl">{tdee.toLocaleString()}</b>
                                    </p>
                                    ...
                                    <p className="text-sm mt-2 text-left">
                                      Based on the <b>{formulaLabel}</b>, you have a total daily energy expenditure of{' '}
                                      <b>{tdee.toLocaleString()}</b> calories, which is{' '}
                                      <b>{weeklyCalories.toLocaleString()}</b> calories per week.
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="lg:w-1/2 pl-4 mt-3 text-sm border-l border-gray-300">
                                  <div className="flex items-center justify-between p-2 border-b border-gray-300">
                                    <p className="flex items-center gap-2">
                                      <img   src="/images/tdee_cal.svg" alt="calorie image for tdee" className="w-6 h-6" />
                                      <b>{data?.payload?.tech_lang_keys['75']}</b>
                                    </p>
                                    <p><b>{data?.payload?.tech_lang_keys['76']}</b></p>
                                  </div>
                                  ...
                                  {[
                                    ...
                                    <div
                                      ...
                                      className={`flex items-center justify-between p-2 border-b border-gray-300 ${
                                        activity === data?.payload?.tech_lang_keys[label] ? 'bg-blue-100' : ''
                                      }`}
                                    >
                            ...
                            <p className="text-center text-lg mt-3">
                              <strong className="text-blue-600">{data?.payload?.tech_lang_keys['77']}</strong>
                            </p>

                            <div className="w-full">
                              <div className="w-full flex justify-center" style={{ height: '300px' }}>
                                <Doughnut data={chartData} options={chartOptions}    key={JSON.stringify(chartData)} />
                              </div>
                            </div>
                          </div>
                            ...
                            <p className="mt-3 text-lg col-span-12  ">
                              <strong id="bmiDetails">
                                ...
                                <b
                                  ...
                                  title={
                                    bmiCategory === 'Underweight' ? '18.5 ' + data?.payload?.tech_lang_keys['18'] :
                                    bmiCategory === 'Normal Weight' ? '18.5 - 24.99' :
                                    bmiCategory === 'Overweight' ? '25 - 29.99' :
                                    bmiCategory === 'Obesity' ? '30 - 34.99' :
                                    bmiCategory === 'Severe Obesity' ? '35+' : ''
                                  }
                                >
                                  {data?.payload?.tech_lang_keys[
                                    bmiCategory === 'Underweight' ? '9' :
                                    bmiCategory === 'Normal Weight' ? '10' :
                                    bmiCategory === 'Overweight' ? '11' :
                                    bmiCategory === 'Obesity' ? '12' :
                                    bmiCategory === 'Severe Obesity' ? '13' : ''
                                  ]}
                                </b>
                            ...
                            <div className="flex flex-col col-span-12 lg:flex-row gap-6">
                              ...
                              <div className="w-full lg:w-1/2 mt-3">
                                  <div className="flex items-center justify-center p-2 bg-red-600 rounded-t-lg gap-2">
                                      <img src='/images/tdee_apple.svg'  alt="apple image for tdee" className="w-5 h-5" decoding="async" loading="lazy"/>
                                      <p className="text-white">{data?.payload?.tech_lang_keys['78']}</p>
                                  </div>
                                  ...
                                  <div className="w-full lg:w-1/2 mt-3">
                                      <div className="flex items-center justify-center p-2 bg-green-600 rounded-t-lg gap-2">
                                          <img   src='/images/tdee_arm.svg'  alt="arm image for tdee" className="w-5 h-5" decoding="async" loading="lazy"/>
                                          <p className="text-white">{data?.payload?.tech_lang_keys['80']}</p>
                                      </div>
                                      <div className="w-full text-sm px-2 border border-blue-600 rounded-b-lg">
                                          {/* Mild Gain */}
                                          <div className="flex items-center justify-between py-2 px-3 border-b border-gray-400">
                     ...
                     <p className="mt-3 text-lg"><b>{ data?.payload?.tech_lang_keys[52] }</b></p>
                    <div className="lg:w-7/12 mt-2 flex items-center p-2 text-sm rounded-lg flex-wrap bg-white shadow-inner gap-4">
                      {[
                        { key: 'maintenance', label: data?.payload?.tech_lang_keys['m1'], datas: data?.payload?.tech_lang_keys['m1_des'] },
                        { key: 'cutting', label: data?.payload?.tech_lang_keys['m2'], datas: data?.payload?.tech_lang_keys['m2_des'] },
                        { key: 'bulking', label: data?.payload?.tech_lang_keys['m3'], datas: data?.payload?.tech_lang_keys['m3_des'] },
                        { key: 'custom', label: data?.payload?.tech_lang_keys[53], datas: "custom" }
                      ].map(tab => (
                        <p
                          key={tab.key}
                     ...
                     <div className="col-12 font-s-14" id="customDetails">
                        <div className="flex flex-col lg:flex-row gap-4 my-2">
                          ...
                          <div className="lg:w-1/3 w-full">
                            <p className="flex justify-between">
                              <strong>{data?.payload?.tech_lang_keys['pro']}</strong>
                        ...
                        onChange={(e) => updateMacro('protein', e.target.value)}
                          ...
                          <div className="lg:w-1/3 w-full lg:px-3 mt-1 lg:mt-0">
                            <p className="flex justify-between">
                              <strong>{data?.payload?.tech_lang_keys['fat']}</strong>
                        ...
                        onChange={(e) => updateMacro('fat', e.target.value)}
                          ...
                          <div className="lg:w-1/3 w-full mt-1 lg:mt-0">
                            <p className="flex justify-between">
                              <strong>{data?.payload?.tech_lang_keys['carb']}</strong>
                        ...
                        <div className="lg:w-1/2 mx-auto flex justify-between flex-wrap mt-4">
                          <div>
                            <div className="flex items-center gap-1">
                              <img src="/images/chart_pro.png" alt="protein" width="15px" height="15px" />
                              <p className="text-[#E94442]">{data?.payload?.tech_lang_keys['pro']}</p>
                            </div>
                          ...
                          <div>
                            <div className="flex items-center gap-1">
                              <img src="/images/chart_fat.png" alt="fat" width="15px" height="15px" />
                              <p className="text-[#E7A827]">{data?.payload?.tech_lang_keys['fat']}</p>
                            </div>
                          ...
                          <div>
                            <div className="flex items-center gap-1">
                              <img src="/images/chart_carb.png" alt="carb" width="15px" height="15px" />
                              <p className="text-[#38a169]">{data?.payload?.tech_lang_keys['carb']}</p>
                            </div>
                        ...
                        {/* Chart */}
                        <div className="w-[300px] mx-auto mt-5" id="custom_moderate">
                        ...
                        <p className="text-sm mt-3" id="macroLine">
                          {
                            tabs.map(tab => tab.key === activeTab && (
                              <span key={tab.key}>
                                {tab.datasBefore} <strong className="dailyCalories">{tab.caloriesValue}</strong> {tab.datasAfter}
                              </span>
                            ))
                          }
                        </p>
                          ...
                          <div className="flex flex-wrap">
                            ...
                            <div className="md:w-1/3 w-full mt-2 pr-3 md:border-r md:border-gray-300">
                              <p className="mb-2">
                                <strong>{data?.payload?.tech_lang_keys['moderate']} (30/35/35)</strong>
                              </p>
                              <div className="flex justify-between">
                                <div>
                                  <div className="flex items-center gap-1">
                                    ...
                                    />
                                    <p className="text-red-600">{data?.payload?.tech_lang_keys['pro']}</p>
                                  </div>
                                  ...
                                  <div className="mt-2">
                                    <div className="flex items-center gap-1">
                                      <img
                                        ...
                                        className="w-4 h-4 object-contain"
                                      />
                                      <p className="text-yellow-500">{data?.payload?.tech_lang_keys['fat']}</p>
                                    </div>
                                    <div className="mt-1 text-yellow-500">{m.mod_fat}</div>
                                  </div>
                                  ...
                                  <div className="mt-2">
                                    <div className="flex items-center gap-1">
                                      <img
                                        ...
                                        className="w-4 h-4 object-contain"
                                      />
                                      <p className="text-green-400">{data?.payload?.tech_lang_keys['carb']}</p>
                                    </div>
                                    <div className="mt-1 text-green-400">{m.mod_carb}</div>
                                  </div>
                                </div>

                                <div className="mt-2" style={{ height: '200px', width: '150px' }}>
                                  <Doughnut data={chartData2[activeTab]} options={options} />
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="md:w-1/3 w-full mt-2 px-3 md:border-r md:border-gray-300">
                              <p className="mb-2">
                                <strong>{data?.payload?.tech_lang_keys['lower']} (40/40/20)</strong>
                              </p>
                              <div className="flex justify-between">
                                <div>
                                  <div className="flex items-center gap-1">
                                  ...
                                  <div className="mt-2">
                                    ...
                                    <div className="mt-1 text-yellow-500">{m.low_fat}</div>
                                  </div>
                                  ...
                                  <div className="mt-2">
                                    ...
                                    <div className="mt-1 text-green-400">{m.low_carb}</div>
                                  </div>
                                </div>
                            ...
                            <div className="md:w-1/3 w-full mt-2 pl-3">
                              <p className="mb-2">
                                <strong>{data?.payload?.tech_lang_keys['high']} (30/20/50)</strong>
                              </p>
                              <div className="flex justify-between">
                                <div>
                                  <div className="flex items-center gap-1">
                                  ...
                                  <div className="mt-1 text-red-600">{m.high_pro}</div>
                                  ...
                                  <div className="mt-2">
                                    ...
                                    <div className="mt-1 text-yellow-500">{m.high_fat}</div>
                                  </div>
                                  ...
                                  <div className="mt-2">
                                    ...
                                    <div className="mt-1 text-green-400">{m.high_carb}</div>
                                  </div>
                                </div>
