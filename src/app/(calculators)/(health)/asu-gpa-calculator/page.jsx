...
const AsuGpaCalculator = () => {
  ...
  const [calculateAsuGpa, { isLoading: calculateGpaLoading }] =
    useAsuGpaCalculatorMutation();
  ...
  const removeSemester = (semesterIndex) => {
    if (formData.semesters.length > 1) {
      const newSemesters = formData.semesters.filter(
  ...
  const handleSubmit = async (e) => {
      ...
      for (const course of semester.courses) {
        if (course.courseName || course.credits || course.grade) {
          if (!course.credits || !course.grade) {
            setFormError(
              "Please fill all credits and grade fields for courses"
            );
            return;
          }
          hasData = true;
    ...
    if (!hasData) {
      setFormError(
        "Please fill in at least one course or current GPA information"
      );
      return;
    }
    ...
    const submissionData = {
      ...
      creditsCompleted: formData.creditsCompleted
        ? parseFloat(formData.creditsCompleted)
        : "",
        ...
        .map((semester) => ({
          ...
          courses: semester.courses
            ...
            .map((course) => ({
              courseName:
                course.courseName ||
                `Course ${Math.random().toString(36).substr(2, 5)}`,
    ...
    setFormError("");
    try {
      const response = await calculateAsuGpa(submissionData).unwrap();
          ...
          <div className="lg:w-[90%] md:w-[90%] w-full mx-auto">
            <div className="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
              ...
              <div className="col-span-12">
                <div className="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
                  <div className="col-span-12 current_gpa bg-[#2845F5] text-white p-2 rounded-lg">
                    <strong
                      className="col-8 cursor-pointer"
                      onClick={() => setShowCurrentGPA(!showCurrentGPA)}
                    >
                      {data?.payload?.tech_lang_keys?.["14"] || "Current"}{" "}
                      {data?.payload?.tech_lang_keys?.["4"] || "GPA"}
                    </strong>
                  </div>
                  <div
                    className={`col-span-12 row current_inp ${
                      showCurrentGPA ? "block" : "hidden"
                    }`}
                  >
                    <div className="grid grid-cols-12 mt-3 gap-2 md:gap-4 lg:gap-4">
              ...
              {formData.semesters.map((semester, semesterIndex) => (
                <div key={semesterIndex} className="col-span-12 overflow-auto">
                  <div className="semester bordered rounded-lg p-3 mt-2 md:w-auto w-[400px]">
                    ...
                    <div
                      className="addCourse mt-2"
                      id={`accept_row${semesterIndex + 1}`}
                    >
                      <ul className="get_html">
                        {semester.courses.map((course, courseIndex) => (
                          <li
                            key={courseIndex}
                            className="row relative grid grid-cols-12 gap-2 md:gap-4 lg:gap-4 mb-3"
                      ...
                      <div className="col-12 pb-2 mt-3 flex flex-wrap gap-2">
                        <button
                          type="button"
                          className="col-6 col-md-4 units_active p-3 border rounded-lg cursor-pointer add_course bg-[#2845F5] text-white"
                          onClick={() => addCourse(semesterIndex)}
                        >
                          <strong className="flex items-center justify-center">
                            <span className="md:text-[18px] text-[14px] pe-2">+</span>
                        ...
                        {semesterIndex === formData.semesters.length - 1 && (
                          <button
                            type="button"
                            className="col-6 col-md-4 units_active p-3 border rounded-lg cursor-pointer add_semester bg-[#2845F5] text-white"
                            onClick={addSemester}
                          >
                            <strong className="flex items-center justify-center">
                              <span className="md:text-[18px] text-[14px] pe-2">+</span>
          ...
          <div className="w-full result mx-auto p-4 lg:p-8 md:p-8 result_calculator rounded-lg space-y-6">
            ...
            <div className="rounded-lg flex items-center justify-center">
              <div className="w-full mt-3">
                ...
                {result.semesters &&
                  result.semesters.map((semester, semesterIndex) => (
                      ...
                      <div className="w-full overflow-auto mt-2">
                        <table className="w-full border-collapse border border-gray-300">
                          ...
                          <tfoot>
                            <tr className="">
                              <td
                                className="p-3 border-b border-gray-300 font-semibold text-right"
                                colSpan="2"
                              >
                                <strong>
                              ...
                              <td className="p-3 border-b border-gray-300 text-center font-semibold text-blue-600">
                            ...
                            <tr className="">
                              ...
                              <td className="p-3 border-b border-gray-300 text-center"></td>
                            </tr>
                          </tfoot>
                        </table>
                      </div>
