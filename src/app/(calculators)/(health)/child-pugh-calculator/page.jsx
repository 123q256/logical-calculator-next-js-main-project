"use client";

import React, { useEffect, useState } from "react";
import { usePathname } from "next/navigation";
import {
  useGetSingleCalculatorDetailsMutation,
  useChildPughCalculatorMutation,
} from "../../../../redux/services/calculator/calculatorApi";

import { toast } from "react-toastify";
import ResultActions from "../../../../components/Calculator/ResultActions";
import CalculatorFeedback from "../../../../components/Calculator/CalculatorFeedback";
import Calculator from "../../Calculator";
import { getUserCurrency } from "../../../../components/Calculator/GetCurrency"; //currency import class
import ResetButton from "../../../../components/Calculator/ResetButton";
import Button from "../../../../components/Calculator/Button";

const ChildPughScoreCalculator = () => {
  const pathname = usePathname();
  const parts = pathname.split("/").filter(Boolean); // remove empty strings

  let url = "";

  if (parts.length === 1) {
    // sirf ek part
    url = parts[0]; // "age-calculator"
  } else {
    // do ya zyada parts
    url = parts[0] + "/" + parts[1]; // "de/age-calculator"
  }
  const [getSingleCalculatorDetails, { data, error, isLoading }] =
    useGetSingleCalculatorDetailsMutation();
  const handleFetchDetails = async () => {
    try {
      // Call the mutation with the `tech_calculator_link`
      await getSingleCalculatorDetails({ tech_calculator_link: url });
    } catch (err) {
      console.error("Error fetching calculator details:", err);
    }
  };

  useEffect(() => {
    handleFetchDetails();
  }, [url]);

  const [formData, setFormData] = useState({
    tech_a: 2,
    tech_e: 1,
    tech_b: 2,
    tech_as: 1,
    tech_i: 3,
  });

  const [result, setResult] = useState(null);
  const [formError, setFormError] = useState("");

  // RTK mutation hook
  const [
    calculateEbitCalculator,
    { isLoading: roundToTheNearestLoading, isError, error: calculateLoveError },
  ] = useChildPughCalculatorMutation();

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData((prevData) => ({ ...prevData, [name]: value }));
    setResult(null);
    setFormError(null);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();

    if (
      !formData.tech_a ||
      !formData.tech_e ||
      !formData.tech_b ||
      !formData.tech_as ||
      !formData.tech_i
    ) {
      setFormError("Please fill in field.");
      return;
    }

    setFormError("");
    try {
      const response = await calculateEbitCalculator({
        tech_a: formData.tech_a,
        tech_e: formData.tech_e,
        tech_b: formData.tech_b,
        tech_as: formData.tech_as,
        tech_i: formData.tech_i,
      }).unwrap();
      setResult(response?.payload); // Assuming the response has 'lovePercentage'
      toast.success("Successfully Calculated");
    } catch (err) {
      setFormError("Error in calculating.");
      toast.error("Error in calculating.");
    }
  };

  // Handle reset form
  const handleReset = () => {
    setFormData({
      tech_a: 2,
      tech_e: 1,
      tech_b: 2,
      tech_as: 1,
      tech_i: 3,
    });
    setResult(null);
    setFormError(null);
  };
  // currency code
  const [currency, setCurrency] = useState({
    code: "USD",
    symbol: "$",
    name: "US Dollar",
  });

  useEffect(() => {
    const fetchCurrency = async () => {
      const result = await getUserCurrency();
      if (result) {
        setCurrency(result);
      }
    };

    fetchCurrency();
  }, []);
  // currency code

  return (
    <Calculator
      isLoading={isLoading}
      data={data}
      links={[
        { name: "Home", path: "/" },
        {
          name: data?.payload?.tech_cal_cat,
          path: "/" + data?.payload?.tech_cal_cat,
        },
        {
          name: data?.payload?.tech_calculator_title,
          path: pathname, // This will use the current path dynamically
        },
      ]}
    >
      <form className="row" onSubmit={handleSubmit}>
        <div className="w-full mx-auto p-4 lg:p-8 md:p-8 input_form rounded-lg  space-y-6 mb-3">
          {formError && (
            <p className="text-red-500 text-lg font-semibold w-full">
              {formError}
            </p>
          )}

          <div className="lg:w-[60%] md:w-[80%] w-full mx-auto ">
            <div className="grid grid-cols-12   gap-2 md:gap-3 lg:gap-3">
              <div className="col-span-12 ">
                <label htmlFor="tech_b" className="label">
                  {data?.payload?.tech_lang_keys["b"]}:
                </label>
                <div className="mt-2">
                  <select
                    className="input"
                    aria-label="select"
                    name="tech_b"
                    id="tech_b"
                    value={formData.tech_b}
                    onChange={handleChange}
                  >
                    <option value="1">&lt;2 mg/dL (&lt;34.2 µmol/L)</option>
                    <option value="2">2-3 mg/dL (34.2-51.3 µmol/L)</option>
                    <option value="3">&gt;3 mg/dL (&gt;51.3 µmol/L)</option>
                  </select>
                </div>
              </div>
              <div className="col-span-12 md:col-span-6 lg:col-span-6">
                <label htmlFor="tech_a" className="label">
                  {data?.payload?.tech_lang_keys["a"]}:
                </label>
                <div className="mt-2">
                  <select
                    className="input"
                    aria-label="select"
                    name="tech_a"
                    id="tech_a"
                    value={formData.tech_a}
                    onChange={handleChange}
                  >
                    <option value="1">3.5 g/dL (&gt;35 g/L)</option>
                    <option value="2">2.8-3.5 g/dL (28-35 g/L)</option>
                    <option value="3">&lt;2.8 g/dL (&lt;28 g/L)</option>
                  </select>
                </div>
              </div>
              <div className="col-span-12 md:col-span-6 lg:col-span-6">
                <label htmlFor="tech_i" className="label">
                  {data?.payload?.tech_lang_keys["i"]}:
                </label>
                <div className="mt-2">
                  <select
                    className="input"
                    aria-label="select"
                    name="tech_i"
                    id="tech_i"
                    value={formData.tech_i}
                    onChange={handleChange}
                  >
                    <option value="1">&lt;1.7</option>
                    <option value="2">1.7-2.2</option>
                    <option value="3">2.2</option>
                  </select>
                </div>
              </div>
              <div className="col-span-12 md:col-span-6 lg:col-span-6">
                <label htmlFor="tech_as" className="label">
                  {data?.payload?.tech_lang_keys["as"]}:
                </label>
                <div className="mt-2">
                  <select
                    className="input"
                    aria-label="select"
                    name="tech_as"
                    id="tech_as"
                    value={formData.tech_as}
                    onChange={handleChange}
                  >
                    <option value="1">
                      {data?.payload?.tech_lang_keys["a1"]}
                    </option>
                    <option value="2">
                      {data?.payload?.tech_lang_keys["a2"]}
                    </option>
                    <option value="3">
                      {data?.payload?.tech_lang_keys["a3"]}
                    </option>
                  </select>
                </div>
              </div>
              <div className="col-span-12 md:col-span-6 lg:col-span-6">
                <label htmlFor="tech_e" className="label">
                  {data?.payload?.tech_lang_keys["e"]}:
                </label>
                <div className="mt-2">
                  <select
                    className="input"
                    aria-label="select"
                    name="tech_e"
                    id="tech_e"
                    value={formData.tech_e}
                    onChange={handleChange}
                  >
                    <option value="1">
                      {data?.payload?.tech_lang_keys["e1"]}
                    </option>
                    <option value="2">
                      {data?.payload?.tech_lang_keys["e2"]}
                    </option>
                    <option value="3">
                      {data?.payload?.tech_lang_keys["e3"]}
                    </option>
                  </select>
                </div>
              </div>
            </div>
          </div>

          <div className="flex justify-center gap-3 mb-6 mt-10">
            <Button type="submit" isLoading={roundToTheNearestLoading}>
              {data?.payload?.tech_lang_keys["calculate"]}
            </Button>
            {result && (
              <ResetButton type="button" onClick={handleReset}>
                {data?.payload?.tech_lang_keys["locale"] === "en"
                  ? "RESET"
                  : data?.payload?.tech_lang_keys["reset"] || "RESET"}
              </ResetButton>
            )}
          </div>
        </div>
        {roundToTheNearestLoading ? (
          <div className="w-full mx-auto p-4 lg:p-8 md:p-8 result_calculator rounded-lg  space-y-6 result">
            <div className="animate-pulse">
              <div className=" w-full h-[30px] bg-gray-300 animate-pulse rounded-[10px] mb-4"></div>
              <div className="w-[75%] h-[20px] bg-gray-300 animate-pulse rounded-[10px] mb-3"></div>
              <div className="w-[50%] h-[20px] bg-gray-300 animate-pulse rounded-[10px] mb-3"></div>
              <div className="w-[25%] h-[20px] bg-gray-300 animate-pulse rounded-[10px]"></div>
            </div>
          </div>
        ) : (
          result && (
            <>
              <div className="w-full mx-auto p-4 lg:p-8 md:p-8 result_calculator rounded-lg space-y-6 result">
                <div>
                  <ResultActions lang={data?.payload?.tech_lang_keys} />

                  <div className="rounded-lg  flex items-center justify-center">
                    <div className="w-full mt-3">
                      <div className="w-full">
                        <p className="w-full text-[20px] mt-2">
                          <strong>Child Pugh Score Calculator</strong>
                        </p>

                        <p className="w-full text-[28px]">
                          <strong className="text-[#119154]">
                            {result?.tech_ans ? (
                              result.tech_ans
                            ) : (
                              <>
                                0.0{" "}
                                <span className="text-[#119154] text-[25px]">
                                  Points
                                </span>
                              </>
                            )}
                          </strong>
                        </p>

                        <p className="w-full text-[25px] mt-2">
                          <strong className="font-s-21">
                            {data?.payload?.tech_lang_keys?.class} :
                          </strong>{" "}
                          <strong>{result?.tech_class || "Nan"}</strong>
                        </p>

                        <p className="w-full text-[18px] mt-2">
                          {result?.tech_ansa ||
                            data?.payload?.tech_lang_keys?.life}
                        </p>

                        <div className="col-10 border-t my-3"></div>

                        <p className="w-full text-[18px]">
                          {result?.tech_ansb ||
                            data?.payload?.tech_lang_keys?.inter}
                        </p>

                        <div className="col-10 border-t my-3"></div>

                        <p className="w-full text-[18px]">
                          <span>{data?.payload?.tech_lang_keys?.one_y} :</span>{" "}
                          <strong>
                            {result?.tech_percent1
                              ? `${result.tech_percent1} %`
                              : "00 %"}
                          </strong>
                        </p>

                        <div className="col-10 border-t my-3"></div>

                        <p className="w-full text-[18px]">
                          <span>{data?.payload?.tech_lang_keys?.two_y} :</span>{" "}
                          <strong>
                            {result?.tech_percent2
                              ? `${result.tech_percent2} %`
                              : "00 %"}
                          </strong>
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </>
          )
        )}
      </form>
      {result && (
        <CalculatorFeedback calName={data?.payload?.tech_calculator_title} />
      )}
    </Calculator>
  );
};

export default ChildPughScoreCalculator;
