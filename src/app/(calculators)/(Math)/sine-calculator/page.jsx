...
const SineCalculator = () => {
  ...
  const [formData, setFormData] = useState({
    tech_angle: 45,
    tech_angle_unit: "deg", //deg  pirad  rad
  });
  ...
  const sin = parseFloat(result?.tech_sin);
  ...
  // Normalize key if negative
  const sinKey = Object.keys(table).find(
    (key) => parseFloat(key) === Math.abs(sin)
  );
  let val = sinKey ? table[sinKey] : null;
  if (val && sin < 0) val = `-${val}`;
                              ...
                              {unit === "deg" && val && (
                                <tr>
                                  <td className="py-2 border-b" width="60%">
                                    <strong>{`sin(${angle}${degSymbol})`}</strong>
                                  </td>
