...
const CotangentCalculator = () => {
  ...
  const [formData, setFormData] = useState({
    tech_angle: 60,
    tech_angle_unit: "deg", // use "deg", "pirad", or "rad"  mrad
  });
  ...
  const angleUnit = formData?.tech_angle_unit;
  const cot = parseFloat(result?.tech_cot)?.toFixed(8);
  const degSymbol =
    angleUnit === "deg" ? "°" : angleUnit === "pirad" ? " * π" : "";

  const table = {
    1.73205081: "\\sqrt{3}",
    "-1.73205081": "-\\sqrt{3}",
    0.57735027: "\\frac{\\sqrt{3}}{3}",
    "-0.57735027": "-\\frac{\\sqrt{3}}{3}",
  };

  let matchedVal = "";
  Object.entries(table).forEach(([key, val]) => {
    const fixedKey = parseFloat(key).toFixed(8);
    if (cot === fixedKey || cot === (-parseFloat(key)).toFixed(8)) {
      matchedVal = cot < 0 ? `-${val.replace(/^-/, "")}` : val;
    }
  });
                      ...
                      <div className="w-full">
                        <div className="w-full md:w-[90%] lg:w-[60%] overflow-auto mt-2">
                          <table className="w-full text-[16px]">
                            <tbody>
                              {angleUnit === "deg" && matchedVal && (
                                <tr>
                                  <td className="py-2 border-b" width="60%">
                                    <strong>{`cot(${angle}${degSymbol})`}</strong>
                                  </td>
                                  <td className="py-2 border-b">
                                    <BlockMath math={matchedVal} />
                                  </td>
                                </tr>
                              )}
                              <tr>
...
export default CotangentCalculator;
