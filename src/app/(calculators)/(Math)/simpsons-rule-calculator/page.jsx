...
const SimpsonsRuleCalculator = () => {
  ...
  // result
  const showSteps = result?.tech_steps || [];
  const limitValues = result?.tech_limit_a || [];
  const f = result?.tech_enter;
  const x = result?.tech_with;
  const n = parseInt(result?.tech_n);
  const delta = result?.tech_diff;
  ...
  for (let i = 0; i < showSteps.length - 1; i++) {
    const a_i = limitValues[i];
    const val = showSteps[i];
    const replaced = f.replaceAll(x, a_i);
    let stepLatex = "";

    if (i === 0 || i === showSteps.length - 2) {
      stepLatex = `f(${x}_${i}) = f(${a_i}) = ${replaced} = ${val}`;
    } else if (ev === 0) {
      stepLatex = `4f(${x}_${i}) = 4f(${a_i}) = 4(${replaced}) = ${val}`;
      ev = 1;
    } else {
      stepLatex = `2f(${x}_${i}) = 2f(${a_i}) = 2(${replaced}) = ${val}`;
      ev = 0;
    }

    renderedSteps.push(
      <p className="mt-3" key={i}>
        <BlockMath math={stepLatex} />
      </p>
    );

    expressionSum += `${val}`;
    if (i !== showSteps.length - 2) expressionSum += " + ";
  }
                    ...
                    <div className="w-full mt-3">
                      <div className="w-full text-[16px] overflow-auto">
                        <div className="mt-3 text-[18px]">
                          <BlockMath
                            math={`\\int\\limits_{${result?.tech_lb}}^{${result?.tech_ub}} ${result?.tech_enter} \\, d${x} = ${result?.tech_res}`}
                          />
                        </div>
                        ...
                        <div className="mt-3">
                          ...
                          <InlineMath>
                            {`\\int\\limits_{${result?.tech_lb}}^{${result?.tech_ub}} ${f} \\, d${x}`}
                          </InlineMath>{" "}
                        ...
                        <div className="mt-3 text-[18px]">
                          <BlockMath
                            math={`\\int\\limits_a^b f(${x})\\, d${x} \\approx \\dfrac{\\Delta ${x}}{3}(f(${x}_0) + 4f(${x}_1) + 2f(${x}_2) + ... + f(x_n))`}
                          />
                        </div>

                        <div className="mt-3">
                          {data?.payload?.tech_lang_keys["9"]}{" "}
                          <BlockMath math={`\\Delta ${x} = \\dfrac{b-a}{n}`} />
                        </div>

                        <div className="mt-3">
                          {data?.payload?.tech_lang_keys["10"]}{" "}
                          <BlockMath
                            math={`a=${result?.tech_lb}, b=${result?.tech_ub}, n=${n}`}
                          />
                        </div>
                        ...
                        <div className="mt-3">
                          ...
                          <BlockMath
                            math={`\\Delta ${x} = \\dfrac{${result?.tech_ub}-${result?.tech_lb}}{${n}} = ${delta}`}
                          />
                        </div>
                        ...
                        {/* Render calculated steps */}
                        {renderedSteps}

                        <div className="mt-3">
                          {data?.payload?.tech_lang_keys["17"]}{" "}
                          <BlockMath
                            math={`\\dfrac{\\Delta ${x}}{3} = ${delta / 3}`}
                          />
                        </div>

                        <div className="mt-3 overflow-auto">
                          <BlockMath
                            math={`\\dfrac{${delta}}{3}(${expressionSum}) = ${result?.tech_res}`}
                          />
                        </div>
                        ...
                        <div className="mt-3">
                          <BlockMath
                            math={`${result?.tech_int} = ${result?.tech_intv}`}
                          />
                        </div>
                        ...
                        <div className="mt-3">
                          <BlockMath
                            math={`\\left| \\varepsilon \\right| = \\left| \\dfrac{${Number(
                              result?.tech_intv
                            ).toFixed(2)} - ${Number(result?.tech_res).toFixed(
                              2
                            )}}{${Number(result?.tech_intv).toFixed(
                              2
                            )}} \\right| \\approx ${Number(
                              result?.tech_errorans
                            ).toFixed(5)} = ${Number(
                              result?.tech_errorans * 100
                            ).toFixed(2)}\\%`}
                          />
                        </div>
                      </div>
