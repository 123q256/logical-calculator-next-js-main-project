...
const SecondDerivativeCalculator = ({ html }) => {
  ...
  const handleClick = (i) => {
    setVisibleStep((prev) => ({ ...prev, [i]: !prev[i] }));
  };

  useEffect(() => {
    Object.keys(visibleStep).forEach((key) => {
      if (visibleStep[key] && containerRefs.current[key]) {
                      ...
                      <div className="w-full text-[16px]">
                        {result?.tech_final_res &&
                          (() => {
                            ...
                            for (
                              let i = 1;
                              i < result.tech_final_res.length;

                            ) {
                              const funcTex = `f${j}(x)`;
                              const resultTex = String(
                                result.tech_final_res[i]
                              );
                              const explanationTitle =
                                data?.payload?.tech_lang_keys["7"];
                              const explanationTex = String(
                                result.tech_final_res[i + 1]
                              );
                              const stepHTML = result.tech_final_res[i + 2];

                              const stepIndex = i + 2; // ✅ consistent and fixed reference for this block
                              ...
                              elements.push(
                                <React.Fragment key={stepIndex}>
                                  ...
                                  <p className="mt-3 text-[18px] font-bold">
                                    <InlineMath
                                      math={`${funcTex} ${data?.payload?.tech_lang_keys["3"]}`}
                                    />
                                  </p>

                                  {/* Result */}
                                  <p className="mt-3 text-[18px]">
                                    <InlineMath math={resultTex} />
                                  </p>
                                  ...
                                  {/* Explanation */}
                                  <p className="mt-3 text-[18px]">
                                    <InlineMath math={explanationTex} />
                                  </p>
                                  ...
                                  <div className="w-full mt-3">
                                    <button
                                      type="button"
                                      onClick={() => handleClick(stepIndex)} // ✅ use fixed index
                                      className="calculate repeat px-6 py-3 font-semibold text-[#ffffff] bg-[#2845F5] cursor-pointer text-[14px] rounded-lg focus:outline-none focus:ring-2"
                                      style={{
                                        fontSize: "16px",
                                        padding: "10px",
                                        cursor: "pointer",
                                  ...
                                  {/* Step Explanation HTML */}
                                  <div
                                    className={`w-full mt-3 res_step ${
                                      visibleStep[stepIndex] ? "" : "hidden"
                                    }`}
                                    step_cal={stepIndex}
                                    ref={(el) =>
                                      (containerRefs.current[stepIndex] = el)
                                    }
                                  >
                                    <p
                                      className="mt-3"
