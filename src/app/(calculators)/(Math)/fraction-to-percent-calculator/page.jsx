...
const FractionToPercentCalculator = () => {
  ...
  const techG = result?.tech_g || "";
  const techUpr = result?.tech_upr || "";
  const techBtm = result?.tech_btm || "";
                    ...
                    <div className="w-full result p-3 radius-10 mt-3">
                      <div className="row">
                        <div className="col-12 text-[16px] md:text-[20px] overflow-auto">
                          <p className="mt-3 text-[16px] md:text-[20px]">
                            <InlineMath
                              math={`${n1}\\dfrac{${n2}}{${d1}} = ${
                                techAns * 100
                              }\\%`}
                          ...
                          <p className="mt-3">
                            ...
                            <InlineMath
                              math={`\\dfrac{${totalN}}{${totalD}}`}
                            />
                          </p>
                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys["step"]} # 2 ={" "}
                            <InlineMath
                              math={`\\dfrac{${totalN}\\div${techG}}{${totalD}\\div${techG}}`}
                            />
                          </p>
                          {techBtm === "1" ? (
                            <p className="mt-3">= {techUpr}</p>
                          ) : (
                            <p className="mt-3">
                              ={" "}
                              <InlineMath
                                math={`\\dfrac{${techUpr}}{${techBtm}}`}
                              />
                            </p>
                          )}
                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys["dec"]}:{" "}
                            {Number(techUpr / techBtm).toFixed(4)}
                          </p>
                          <p className="mt-3">
                            {data?.payload?.tech_lang_keys["per"]}: {techAns} ×
                            100 = {techAns * 100}%
                          </p>
                        </div>
                      </div>
