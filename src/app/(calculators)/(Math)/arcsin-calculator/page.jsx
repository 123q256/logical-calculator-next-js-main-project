...
const ArcsinCalculator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.tech_arcsin || !formData.tech_round) {
    ...
    try {
      const response = await calculateEbitCalculator({
        tech_arcsin: formData.tech_arcsin,
  ...
  const angle = parseFloat(result?.tech_angle) || 0;
  const rad = parseFloat(result?.tech_rad) || 0;
  const roundTo = parseInt(formData?.tech_round) || 2;

  // Degree format (e.g. x°, x°, x°, ...)
  const degAnswers = [
  ...
  // Radian π format (e.g. xπ, xπ, xπ, ...)
  const radDivPi = rad / Math.PI;
  const radAnswers = [
    (radDivPi + -1 * 2).toFixed(roundTo),
    (radDivPi + 0 * 2).toFixed(roundTo),
    (radDivPi + 1 * 2).toFixed(roundTo),
  ];
          ...
          <div className="lg:w-[40%] md:w-[60%] w-full mx-auto ">
            <div className="grid grid-cols-12 mt-3   ">
              <div className="col-span-12">
                <label htmlFor="tech_arcsin" className="label">
                    ...
                    <div className="w-full mt-3">
                      <div className="w-full">
                        ...
                        <div className="w-full text-[16px] mt-3 overflow-auto">
                          ...
                          <p className="mt-2">
                            arcsin {formData?.tech_arcsin} = sin
                            <sup className="font-s-14">-1</sup>{" "}
                            {formData?.tech_arcsin} = {result?.tech_deg}°{" "}
                            {result?.tech_min}' {result?.tech_sec}"
                          </p>

                          <p className="mt-2">
                            = {result?.tech_angle} + k * 360° (k = -1,0,1,...)
                          </p>
                          ...
                          <p className="mt-2">
                            = {result?.tech_rad} rad + k * 2π (k = -1,0,1,...)
                          </p>
