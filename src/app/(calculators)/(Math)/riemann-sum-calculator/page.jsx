...
const RiemannSumCalculator = () => {
  ...
  const integrationType = result?.tech_type;
  const withVar = result?.tech_with;
  const n = result?.tech_n;
  const delta = result?.tech_diff;
  const a = result?.tech_lb;
  const b = result?.tech_ub;
  const limit = result?.tech_limit;
  ...
  const renderSteps = () => {
    ...
    if (integrationType === "1" || integrationType === "2") {
      steps.forEach((val, i) => {
        if (val) {
          sumExpr += `${val}${i < steps.length - 1 ? " + " : ""}`;
          stepOutput.push(
            <p className="mt-3" key={i}>
              <InlineMath
                math={`f(${withVar}_${i}) = f(${
                  limits[i]
                }) = ${result.tech_enter.replace(withVar, limits[i])} = ${val}`}
              />
            </p>
          );
        }
      ...
      return (
        <>
          {stepOutput}
          <p className="mt-3">
            <InlineMath math={`\Delta ${withVar} = ${delta}`} />
          </p>
          <p className="mt-3">
            <InlineMath math={`${delta}(${sumExpr}) = ${result.tech_res}`} />
          </p>
        </>
      );
    }
    ...
    if (integrationType === "3") {
      ...
      steps.forEach((val, i) => {
        if (limits[i + 1] !== undefined) {
          const mid = ((limits[i] + limits[i + 1]) / 2).toFixed(2);
          if (val) {
            // Add to total expression string
            sumExpr += `${val}${i < steps.length - 1 ? " + " : ""}`;

            // Push formatted step to output
            stepOutput.push(
              <p className="mt-3" key={i}>
                <InlineMath
                  math={`f\\left(\\frac{${withVar}_${i}+${withVar}_${
                    i + 1
                  }}{2}\\right) = f\\left(\\frac{${limits[i]}+${
                    limits[i + 1]
                  }}{2}\\right) = f(${mid}) = ${result.tech_enter.replace(
                    withVar,
                    mid
                  )} = ${val}`}
                />
      ...
      return (
        <>
          ...
          <p className="mt-3">
            <InlineMath
              math={`${delta} \\left(${sumExpr}\\right) = ${result.tech_res}`}
            />
          </p>
        </>
      );
    ...
    if (integrationType === "4") {
      ...
      steps.forEach((val, i) => {
        if (val && limits[i + 1] !== undefined) {
          const left = result.tech_enter.replaceAll(withVar, limits[i]);
          const right = result.tech_enter.replaceAll(withVar, limits[i + 1]);
          sumExpr += `${val}${i < steps.length - 1 ? " + " : ""}`;

          stepOutput.push(
            <p className="mt-3" key={i}>
              <InlineMath
                math={`\\frac{f(${withVar}_${i}) + f(${withVar}_${
                  i + 1
                })}{2} = \\frac{f(${limits[i]}) + f(${
                  limits[i + 1]
                })}{2} = \\frac{${left} + ${right}}{2} = ${val}`}
              />
            </p>
          );
        }
                    ...
                    <div className="w-full mt-3">
                      <div className="w-full text-[16px] overflow-auto">
                        <p className="mt-3 text-[18px]">
                          <InlineMath
                            math={`\int\limits_{${a}}^{${b}} ${result.tech_enter}\, d${withVar} = ${result.tech_res}`}
                          />
                        </p>
                        ...
                        <p className="mt-3">
                          ...
                          <InlineMath
                            math={`\int\limits_{${a}}^{${b}} ${result.tech_enter}\, d${withVar}`}
                          />{" "}
