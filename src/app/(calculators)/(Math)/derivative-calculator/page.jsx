...
const DerivativeCalculator = () => {
  ...
  const toggleStep = (index) => {
    setVisibleStep((prev) => (prev === index ? null : index));
  };
  ...
  useEffect(() => {
    ...
    scripts.forEach((script) => {
      ...
      try {
        ...
        script.replaceWith(document.createTextNode(tex)); // fallback
      }
    });
  }, [result?.tech_buffer]); // correct dependency

  const sanitizeAndReplaceMath = (html) => {
    return html.replace(
  ...
  useEffect(() => {
    if (!stepRefs.current || !Array.isArray(stepRefs.current)) return;

    stepRefs.current.forEach((ref) => {
      if (ref) {
        const scripts = ref.querySelectorAll('script[type^="math/tex"]');
  ...
  useEffect(() => {
    if (visibleStep !== null && stepRefs.current[visibleStep]) {
      stepRefs.current[visibleStep].scrollIntoView({ behavior: "smooth" });
    }
  }, [visibleStep]);
                  ...
                  <div className="rounded-lg flex items-center justify-center">
                    <div className="w-full bg-light-blue result p-3 radius-10 mt-3">
                      <div className="w-full overflow-auto">
                        {formData?.tech_how > 1 ? (
                          <>
                            {result?.tech_final_res?.map((res, i) => {
                              if (i % 3 !== 1) return null;

                              const prime = "'".repeat((i - 1) / 3 + 1);
                              const fx = result.tech_final_res[i];
                              const simplified = result.tech_final_res[i + 1];
                              const steps = result.tech_final_res[i + 2];

                              return (
                                <div key={i} className="mb-5">
                                  <div className="mt-3 text-[18px]">
                                    <strong>
                                      <BlockMath math={`f${prime}(x)`} />{" "}
                                  ...
                                  <div className="col-12 mt-3">
                                    <button
                                      ...
                                      className="calculate repeat bg-[#99EA48] rounded-lg px-4 py-2 text-[16px] cursor-pointer"
                                    >
                                  ...
                                  {visibleStep === i && (
                                    <div className="col-12 mt-3 res_step">
                                      <div
                                        className="mt-3"
