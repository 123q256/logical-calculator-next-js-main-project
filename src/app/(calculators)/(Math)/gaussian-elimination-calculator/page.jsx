...
const GaussianEliminationCalculator = () => {
  ...
  const convert = (x) => {
    ...
    if (frac === 0) return [String(x2), 1];

    const den = Math.pow(10, frac.toString().split(".")[1]?.length || 0);
    const num = Math.round(frac * den);
    const g = gcd(num, den);
    const num2 = Math.round(num / g);
    const den2 = Math.round(den / g);
    const top = num2 + y * den2;
    const sign = x2 < 0 ? "-" : "";
  ...
  const renderMatrix = (matrix, matrix22) => {
    ...
    return (
      <BlockMath>{`\\left[\\begin{array}{${"c".repeat(
        matrix22 - 1
      )}|c} ${content} \\end{array}\\right]`}</BlockMath>
    );
  };
  ...
  const renderInitialMatrix = (formData, matrix2, matrix22) => {
    ...
    return (
      <BlockMath>{`\\begin{bmatrix} ${content} \\end{bmatrix}`}</BlockMath>
    );
  };
...
export default GaussianEliminationCalculator;
