...
const CscCalculator = () => {
  ...
  const [formData, setFormData] = useState({
    tech_angle: 12,
    tech_angle_unit: "deg", // use "deg", "pirad", or "rad"
  });
  ...
  const csc = parseFloat(result?.tech_csc)?.toFixed(8);
  const degSymbol =
    angleUnit === "deg" ? "°" : angleUnit === "pirad" ? " * π" : "";

  const table = {
    1.41421356: "\\sqrt{2}",
    1.15470054: "\\frac{2\\sqrt{3}}{3}",
  };

  let matchedVal = "";
  Object.entries(table).forEach(([key, val]) => {
    const fixedKey = parseFloat(key).toFixed(8);
    if (csc === fixedKey || csc === (-parseFloat(key)).toFixed(8)) {
      matchedVal = csc < 0 ? `-${val}` : val;
    }
  });
