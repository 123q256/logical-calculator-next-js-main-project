...
const MidpointRuleCalculator = () => {
  ...
  const limits = result?.tech_limit_a || [];
  let show = "";

  const stepElements = steps.map((value, i) => {
    if (!value) return null;

    const a = limits[i];
    const b = limits[i + 1];
    const mid = ((a + b) / 2).toFixed(5);
    const expression = result?.tech_enter.replaceAll(result?.tech_with, mid);
    ...
    return (
      <div className="mt-3" key={i}>
        <InlineMath
          math={`f\\left(\\frac{${result?.tech_with}_${i}+${
            result?.tech_with
          }_${
            i + 1
          }}{2}\\right) = f\\left(\\frac{${a}+${b}}{2}\\right) = f(${mid}) = ${expression} = ${value}`}
        />
      </div>
    );
  });
                  ...
                  <div className="rounded-lg flex items-center justify-center">
                    <div className="w-full mt-3">
                      <div className="w-full text-[16px] overflow-auto">
                        ...
                        <div className="mt-3">
                          <InlineMath
                            math={`\\int\\limits_{a}^{b} f(${result?.tech_with})\\, d${result?.tech_with} \\approx \\Delta ${result?.tech_with}(f(\\frac{${result?.tech_with}_0+${result?.tech_with}_1}{2}) + f(\\frac{${result?.tech_with}_1+${result?.tech_with}_2}{2}) + \\cdots + f(\\frac{${result?.tech_with}_{result?.tech_n - 2}+${result?.tech_with}_{result?.tech_n - 1}}{2}) + f(\\frac{${result?.tech_with}_{result?.tech_n - 1}+${result?.tech_with}_${result?.tech_n}}{2}))`}
                          />
                        </div>
                        <div className="mt-3">
                        ...
                        <div className="mt-3">
                          <InlineMath math={`a = ${result?.tech_limit} = b`} />
                        </div>
                        <p className="mt-3">
                        ...
                        <div className="mt-3">
                          <InlineMath
                            math={`${result?.tech_diff}(${show}) = ${result?.tech_res}`}
                          />
                        </div>
                      </div>
                    </div>
...
export default MidpointRuleCalculator;
